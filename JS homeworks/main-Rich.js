...
// Question: How can you iterate over a list of products in a shopping cart and calculate the total cost using forEach?
...
const inventory = [
  ...
  { name: "Cheese", expiryDate: "2025-06-15" },
  { name: "Yogurt", expiryDate: "2023-12-20" }
];
...
inventory.forEach(item => {
  if (new Date(item.expiryDate) < today) {
    console.log(`${item.name} is expired.`);
  }
});


// 4. Sending Emails to Users
// Scenario: You need to send a personalized email to each user in an array.

const userz = ["alice@example.com", "bob@example.com", "charlie@example.com"];

users.forEach(email => {
  console.log(`Sending email to ${email}...`);
});

// 5. Processing Student Scores
// Scenario: Determine if students passed or failed based on their scores.
...
// 6. Formatting Dates in a List
// Scenario: Convert raw date strings to a readable format.

const dates = ["2025-04-02", "2024-11-19", "2023-06-22"];
...
// 7. Displaying Products on a Web Page
// Scenario: Dynamically create product elements and display them.
...
// Given an array of objects representing students, log each student's name in uppercase using map and forEach.
...
// students.forEach(student => student.name.toUpperCase())
// console.log(students)


// How would you use map and filter together to return only even numbers, squared?
...
// let squareNum=evenNum.map(number=>number**2)
// console.log(evenNum,squareNum)
...
console.log(hobbies);
// There is a mistake in the following code. What is it, and how do you fix it?
...
// What will be the output of this flatMap example?


const data = ["hello world", "goodbye world"];
const result = data.flatMap(str => str.split(" "));
console.log(result);
// What will be the output of this map example?


// const nums = [1, 2, 3];
// const output = nums.map(num => [num * 2]);
// console.log(output);
// Convert an array of numbers into an array of their square roots using map.


// const numbers = [4, 9, 16, 25];
// What happens if you return undefined inside a map callback?


// const nums = [1, 2, 3];
// const output = nums.map(num => {undefined});
// console.log(output);
// Given an array of objects with names and scores, return only names of students who scored above 80 using filter and map.
...
let above80= students.filter(scores=>scores.score> 80)

let distinction= students.map(scores=>scores.score > 80)

console.log(above80,distinction)
// What will be the output of the following flatMap example?


const arr = [1, 2, 3, 4];
const result = arr.flatMap(num => (num % 2 === 0 ? [num] : []));
console.log(result);
// Rewrite the following forEach loop using map so that it returns a new array.
...
// Edit
// const arr = [[1], [2], [3]];
// console.log(arr.flatMap(x => x.concat(0)));
// Write a function using map that takes an array of numbers and returns an array of their cubes.